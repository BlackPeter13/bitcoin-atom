...
UniValue getrawchangeaddress(const JSONRPCRequest& request)
{
    ...
    CTxDestination dest = GetRawChangeAddress(pwallet, output_type);
    return EncodeDestination(dest);
}
...
static void SendMoney(CWallet * const pwallet, const CTxDestination &address, CAmount nValue, bool fSubtractFeeFromAmount, CWalletTx& wtxNew, const CCoinControl& coin_control)
{
    ...
    if (fWalletUnlockMintOnly) {
        std::string strError = "Error: Wallet unlocked for block minting only, unable to create transaction.";
...
UniValue walletpassphrase(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3) {
        throw std::runtime_error(
            ...
            "3. mintonly           (boolean, optional, default=false) Optional true/false allowing only block minting.\n"
            "\nNote:\n"
    ...
    UniValue ret(UniValue::VOBJ);
    ret.pushKV("mintonly", fWalletUnlockMintOnly);
...
UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    pwallet->AvailableCoins(vecOutputs, !include_unsafe, nullptr, 0, nMinimumAmount, nMaximumAmount, nMinimumSumAmount, nMaximumCount, nMinDepth, nMaxDepth);
